...
- byte ranges that must remain byte-identical after writing
- byte ranges to replace with generated metadata
- size fields to patch after output bytes are built
...
| Group | Meaning |
| --- | --- |
| `copy_ranges` | Source ranges the future builder should copy before patching size fields. |
| `immutable_ranges` | Source ranges that must be byte-identical in the final output, especially the audio payload. |
| `replace_ranges` | Source ranges the future builder should replace with generated bytes. |
| `patch_fields` | Length fields that must be recalculated and patched after output bytes are built. |
